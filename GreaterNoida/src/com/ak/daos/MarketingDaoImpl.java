...
public class MarketingDaoImpl implements MarketingDao {
	...
	public ArrayList<Marketing> retrieveMarketingRecords(ArrayList<String> params) {
		...
		if(!params.isEmpty())
		{
			...
			{
				if(q==null)
					q="From Marketing m where (m."+param.substring(param.indexOf("@")+1)+" like '"+param.substring(0,param.indexOf("@"))+"%' or m."+param.substring(param.indexOf("@")+1)+" like '%"+param.substring(0,param.indexOf("@"))+"' or m."+param.substring(param.indexOf("@")+1)+" like '%"+param.substring(0,param.indexOf("@"))+"%')";
				else
					q=q+" and (m."+param.substring(param.indexOf("@")+1)+" like '"+param.substring(0,param.indexOf("@"))+"%' or m."+param.substring(param.indexOf("@")+1)+" like '%"+param.substring(0,param.indexOf("@"))+"' or m."+param.substring(param.indexOf("@")+1)+" like '%"+param.substring(0,param.indexOf("@"))+"%')";
			}
		}
		else
			q="From Marketing m";
	...
	{    System.out.println("getMarketingRecord(int sno):1");
		for(Iterator it=sessionFactory.getCurrentSession().createQuery("From Marketing marketing where marketing.sno="+sno+"").list().iterator();it.hasNext();)
			return (Marketing)it.next();
		return null;
	}
	...
	public ArrayList<Marketing> retrieveMarketingRecords(String[] snos) {
		...
		session=sessionFactory.getCurrentSession();
		for(String sno:snos)
			for(Iterator it=session.createQuery("From Marketing law where law.sno="+Integer.parseInt(sno)+"").list().iterator();it.hasNext();)
				records.add((Marketing)it.next());
		return records;
	}
